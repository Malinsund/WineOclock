...
Cypress.Commands.add('goToCheckout', () => {
  ...
  cy.get('body').then(($body) => {
    const openCartSidebarButton = $body.find('[data-cy="open-cart-sidebar"]');
    const cartLink = $body.find('[data-cy="cart-link"]');
    if (!cartLink.length && openCartSidebarButton.length) {
      openCartSidebarButton.trigger('click');
    }
  });
...
Cypress.Commands.add('changeQuantity', (direction, to) => {
  cy.get('[data-cy="cart-item"]').each(($element, index) => {
    ...
    cy.wrap($element).find('[data-cy="product-quantity"]').should('contain.text', to);
    cy.wrap($element)
...
Cypress.Commands.add('displaysCartItem', ($element, index) => {
  ...
  cy.wrap($element)
    ...
    .should('exist')
    .invoke('attr', 'src')
    .should('contain', cartItem.image.split('/').pop());
  cy.wrap($element)
  ...
  cy.wrap($element)
    ...
    .should('exist')
    .should('be.visible')
    .should('contain.text', cartItem.quantity);
});
...
Cypress.Commands.add('displaysProduct', (idOrTitle, options) => {
  ...
  cy.get('main').then(($main) => {
    ...
    cy.wrap($element)
      ...
      .should('exist')
      .invoke('attr', 'src')
      .should('contain', product.image.split('/').pop());
    cy.wrap($element)
      .find('[data-cy="product-title"]')
      .should('exist')
      .should('be.visible')
